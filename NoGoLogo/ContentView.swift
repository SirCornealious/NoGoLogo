...
struct ContentView: View {
    ...
    private var apiViewsSection: some View {
        HStack(spacing: 20) {
            if selectedAPIs.contains("xai"), let apiKey = apiKeys.first(where: { $0.type == "xai" })?.key {
                XAIView(prompt: prompt, numberOfImages: numberOfImages, apiKey: apiKey, isLoading: $isLoading, message: $message, trigger: $generateTrigger, parameters: parameterStorage.xaiParameters)
            }
            if selectedAPIs.contains("openai"), let apiKey = apiKeys.first(where: { $0.type == "openai" })?.key {
                OpenAIView(prompt: prompt, numberOfImages: numberOfImages, apiKey: apiKey, isLoading: $isLoading, message: $message, trigger: $generateTrigger, parameters: parameterStorage.openaiParameters)
            }
            if selectedAPIs.contains("gemini"), let apiKey = apiKeys.first(where: { $0.type == "gemini" })?.key {
                GeminiView(prompt: prompt, numberOfImages: numberOfImages, apiKey: apiKey, isLoading: $isLoading, message: $message, trigger: $generateTrigger, parameters: parameterStorage.geminiParameters)
            }
        }
        .frame(maxHeight: 400)
        .padding(.horizontal, 40)
    ...
    func refinePromptWithGrok(prompt: String) async throws -> String {
        guard let apiKey = apiKeys.first(where: { $0.type == "xai" })?.key else {
            LogManager.shared.log("error", "No xAI API key found for Grokify")
            throw NSError(domain: "No API Key", code: 0, userInfo: nil)
        }
        
        LogManager.shared.log("info", "Grokifying prompt: '\(prompt)'")
        ...
        guard let choices = json?["choices"] as? [[String: Any]],
            ...
            LogManager.shared.log("error", "Failed to parse Grok API response")
            throw URLError(.cannotParseResponse)
        }
        
        LogManager.shared.log("info", "Grok API response: '\(content)'")
        return content
    }
}
